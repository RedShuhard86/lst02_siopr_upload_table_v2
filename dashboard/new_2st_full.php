...
<script>
    ...
    function updateHtml(data) {
        ...
        if(data.indexOf('Unauthorized') == -1){
			...
			if(datax.Переменные.ДатаС == datax.Переменные.ДатаСОтключениемПроверок && datax.Переменные.ПредыдущийНомерСессии) {
				$('#ТекстЗаявкиКОтклонениюНажатие').text('Заявки к отклонению ' + datax.Переменные.ПредыдущийНомерСессии + ' сессии');
			} else {
				$('#ТекстЗаявкиКОтклонениюНажатие').text('Заявки к отклонению');
			}
        }else{
